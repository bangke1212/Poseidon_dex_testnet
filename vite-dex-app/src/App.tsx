...
export default function App() {
  ...
  return (
    <div className="min-h-screen relative bg-white">
        <div className="fixed inset-0 z-0 pointer-events-none"><img src={bgWaveNight} alt="" className="w-full h-full object-cover opacity-85" /></div>
      {/* ====== HEADER ====== */}
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-4 lg:py-8"><div key={activeTab} className="animate-[fadeIn_300ms_ease-out]">
        ...
        {activeTab === 'portfolio' && (
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-[#F0F0F0] overflow-hidden">
              <div className="px-5 py-3.5 border-b border-[#F0F0F0]"><h3 className="text-sm font-semibold text-[#1A1A1A] flex items-center gap-2"><Coins size={14} className="text-[#B8A9E8]" /> Your tokens</h3></div>
              <div className="divide-y divide-[#F0F0F0]">
                {chainTokens.map((t: any) => { const value = Number(t.balance || 0) * Number(t.priceUsd || 0); const pct = portfolioStats.totalUsd > 0 ? (value / portfolioStats.totalUsd) * 100 : 0; return (
                  <div key={t.id} className="px-5 py-3.5 hover:bg-[#FAFAF8] transition-colors flex items-center gap-3">
                    ...
                    <div className="text-right"><p className="text-sm font-semibold text-[#1A1A1A]">{fmt(t.balance, 4)} <span className="text-[#9B9B9B] font-normal text-xs">{t.symbol}</span></p><p className="text-[11px] text-[#6B6B6B]">{fmtUsd(value)} \u00b7 {pct.toFixed(1)}%</p></div>
        ...
        {activeTab === 'history' && (
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-[#F0F0F0] overflow-hidden">
              <div className="px-5 py-3.5 border-b border-[#F0F0F0] flex items-center justify-between"><h3 className="text-sm font-semibold text-[#1A1A1A] flex items-center gap-2"><History size={14} className="text-[#B8A9E8]" /> Transaction history</h3><span className="text-[11px] text-[#9B9B9B]">{chainTxs.length} transactions</span></div>
              <div className="divide-y divide-[#F0F0F0]">
